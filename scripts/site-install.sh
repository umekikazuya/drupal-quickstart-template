...
# DB起動待ち
until docker-compose exec drupal-quickstart-db mysqladmin ping -h"localhost" -uroot -proot --silent 2>/dev/null; do
...
docker-compose exec drupal-quickstart-app php -d memory_limit=512M vendor/bin/drush si standard \
  --locale=ja \
